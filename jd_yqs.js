...
} else l1IIil = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...illill($.getdata("CookiesJD") || "[]").map(I1I1II => I1I1II.cookie)].filter(i11ll => !!i11ll);
...
function l1l11(Il1I) {
    try {
        ...
        return console.log(IiIi1i), console.log("京东服务器访问数据为空，请检查自身设备网络情况"), false;
    }
}

function illill(llII1l) {
    if (typeof llII1l == "string") try {
        return JSON.parse(llII1l);
    } catch (illI1I) {
        return console.log(illI1I), $.msg($.name, "", "请勿随意在BoxJs输入框修改内容\n建议通过脚本去获取cookie"), [];
    }
}
            ...
            try {
                if (i11i1) {
                    ...
                    if (I1liiI.code == 0) {
                        ...
                        } else console.log("获得新手红包," + (I1liiI?.["data"]?.["prizeDrawVO"]?.["prizeConfigName"] || 0)), await $.wait(parseInt(Math.random() * 1000 + 1000, 10)), await I1lI11(ii1l, 2);
                    } else I1liiI.code == 402 ? console.log("进入首页失败," + (I1liiI?.["errMsg"] || "")) : console.log("进入首页失败," + (I1liiI?.["errMsg"] || ""));
                }
            } catch (liIIl1) {
...
async function I1lI11(IIiilI, I1lii1) {
    return new Promise(async i11Il1 => {
        ...
        $.post(illii1, async (Iii1i1, IIIlll, llli1I) => {
            try {
                if (Iii1i1) console.log("" + JSON.stringify(Iii1i1)); else {
                    ...
                    } else llli1I.code == 402 ? console.log("浇水失败," + (llli1I?.["errMsg"] || "")) : console.log("浇水失败," + (llli1I?.["errMsg"] || ""));
                }
            } catch (IIiiil) {
...
async function I1I1Il() {
    return new Promise(async i11Iil => {
        ...
        $.post(lIi1ll, async (l1IIiI, IlilIi, I1lilI) => {
            try {
                if (l1IIiI) console.log("" + JSON.stringify(l1IIiI)); else {
                    ...
                    if (I1lilI.code == 0) {
                        ...
                        if (I1lilI.code == 402) console.log("抽奖失败," + (I1lilI?.["errMsg"] || "")); else {
        ...
        $.get(i11Ii1, async (liliIl, liii1l, I1iiII) => {
            try {
                if (liliIl) $.log(liliIl); else {
                    ...
                    if (I1iiII?.["code"] == 0) {
                        ...
                        for (let I1iiI1 = 0; I1iiI1 < i1Ili.length; I1iiI1++) {
                            ...
                            if ($.timeLimitPeriod == null) {
                                if (!$.taskFinished && $.taskType.includes("BROWSE_")) for (let li11l1 = 0; li11l1 < 1; li11l1++) {
                    ...
                    } else console.log("查询任务失败," + (I1iiII?.["errMsg"] || I1iiII?.["msg"] || ""));
                }
            } catch (li1il) {
                $.log(li1il);
            } finally {
...
async function iIlI1I(li1ii, lI1iI1) {
    return new Promise(async iliiiI => {
        ...
        $.post(lIl1Il, async (Ii1IlI, Il1I1I, ll1Il1) => {
            try {
                if (Ii1IlI) console.log("" + JSON.stringify(Ii1IlI)); else {
                    ll1Il1 = JSON.parse(ll1Il1);

                    if (ll1Il1.code == 0) { } else {
                        if (ll1Il1.code == 402) { } else { }
                    }
                }
            } catch (iIIIli) {
        ...
        $.post(I1I11i, async (l1iI1l, III1ll, l1lIIi) => {
            try {
                if (l1iI1l) console.log("" + JSON.stringify(l1iI1l)); else {
                    ...
                    if (l1lIIi.code == 0) { } else {
                        if (l1lIIi.code == 402) { } else { }
                    }
                }
            } catch (iilIiI) {
...
async function IiIl1(Ili1iI) {
    return new Promise(async ll1IlI => {
        ...
        $.post(iilIil, async (illI1i, i1IiII, illI1l) => {
            try {
                if (illI1i) console.log("" + JSON.stringify(illI1i)); else {
                    illI1l = JSON.parse(illI1l);
                    if (illI1l.code == 0) console.log("浏览时间结束"); else illI1l.code == 402 ? console.log("浏览时间失败," + (illI1l?.["errMsg"] || "")) : console.log("浏览时间失败," + (illI1l?.["errMsg"] || ""));
                }
            } catch (I1I111) {
...
async function IIIl() {
    return new Promise(async iIIlIl => {
        ...
        $.post(iilIli, async (llIiiI, IiIilI, lIiIl1) => {
            try {
                if (llIiiI) console.log("" + JSON.stringify(llIiiI)); else {
                    lIiIl1 = JSON.parse(lIiIl1);
                    if (lIiIl1.code == 0) $.drawLotteryNum++, console.log("完成任务,抽奖次数：" + $.drawLotteryNum); else lIiIl1.code == 402 ? console.log("完成任务失败," + (lIiIl1?.["errMsg"] || "")) : console.log("完成任务失败," + (lIiIl1?.["errMsg"] || ""));
                }
            } catch (lIiIlI) {
...
async function IIIi(lIlli1, llIii1, I1IlII) {
    return new Promise(async IiIiil => {
        ...
        $.post(IiII1, async (iIIlII, IlIi1l, IiIiiI) => {
            try {
                if (iIIlII) console.log("" + JSON.stringify(iIIlII)); else {
                    IiIiiI = JSON.parse(IiIiiI);
                    if (IiIiiI.code == 0) console.log("完成任务,获得抽奖次数：" + IiIiiI?.["data"]?.["finishNeed"]), $.drawLotteryNum++; else IiIiiI.code == 402 ? console.log("完成任务失败," + (IiIiiI?.["errMsg"] || "")) : console.log("完成任务失败," + (IiIiiI?.["errMsg"] || ""));
                }
            } catch (lillI) {
                $.logErr(lillI, IlIi1l);
            } finally {
...
function l1IIli(Il11l) {
    ...
    for (i = 0; i < Il11l; i++) illl1i += l1ilI.charAt(Math.floor(Math.random() * IIi1));
...
async function iII1I1(illl1l, Il11I) {
    try {
        ...
        return await li1lli.genAlgo(), body = await li1lli.genUrlParams(Il11I.functionId, Il11I.body), body;
    } catch (iiiIl1) { }
}
...
async function iiiIIi(I1i1I) {
    return new Promise(async Iil1I => {
        ...
        $.get(i1Ii1l, async (IIll, i1Ii1i, iiiIll) => {
            try {
                if (IIll) console.log("" + JSON.stringify(IIll)); else {
                    ...
                    if (iiiIll) {
                        ...
                        } else console.log("摇钱树提现查询奖品：异常:" + JSON.stringify(iiiIll));
                    }
                }
            } catch (iII1I) {
                $.logErr(iII1I, i1Ii1i);
...
async function lIi1iI(i1i1i1, iiiIli, IllIl, IllIi) {
    return new Promise(async iilllI => {
        ...
        $.post(iillil, async (iillii, ilI1iI, l1I1i) => {
            try {
                if (iillii) console.log("" + JSON.stringify(iillii)), console.log($.name + " API请求失败，请检查网路重试"); else {
                    if (l1l11(l1I1i)) {
                        l1I1i = $.toObj(l1I1i);

                        if (l1I1i.code === 0) {
                            if (l1I1i.data.status === "310") console.log("提现现金成功！"); else {
                                console.log("提现现金：失败:" + l1I1i.data.message);
                                if (l1I1i.data.message.includes("上限")) $.txhot = true; else l1I1i.data.message.includes("已存在状态") && (await $.wait(parseInt(Math.random() * 2000 + 5000, 10)), await lIi1iI(i1i1i1, iiiIli, IllIl, IllIi));
                            }
                        } else console.log("提现现金：异常:" + JSON.stringify(l1I1i));
                    }
                }
            } catch (i1i1il) {
    ...
    return new Promise(liiI => {
        ...
        $.post(Ililli, async (lii1, iIii1l, iIliIi) => {
            try {
                ...
                } else l1l11(iIliIi) && (iIliIi = $.toObj(iIliIi), iIliIi.code == 0 ? console.log("兑换红包成功") : console.log("兑换红包失败:" + iIliIi.errMsg));
            } catch (II1i1I) {
...
function lIIi11(iI11l1, IlIii = {}) {
    ...
    if (IlIii.sort) I111l1 = I111l1.sort();

    for (let IlIil of I111l1) {
        let i1i1li = iI11l1[IlIil];
        if (i1i1li && typeof i1i1li === "object") i1i1li = JSON.stringify(i1i1li);
        if (i1i1li && IlIii.encode) i1i1li = encodeURIComponent(i1i1li);
...
async function illiii() {
    ...
    return uuid = Buffer.from(iil1Ii, "utf8").toString("base64"), ep = encodeURIComponent(JSON.stringify({
...
function IIIll1(iIili = "xxxxxxxxxxxxxxxxxxxx") {
    return iIili.replace(/[xy]/g, function (IIil11) {
        ...
        return jdaid = liIIIi.toString(), jdaid;
    });
}
