...
All变量适用
————————————————
入口：[ 3.23~4.10 动感乐活，约惠精彩 (https://lzdz1-isv.isvjcloud.com/dingzhi/divi/active/activity?activityId=dz5ba3b5b44e35b7d350806a736090&shareUuid=752d425cbe0d4918832e7851a11d6724)]
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard3 ? process.env.guaopencard3 : `${guaopencard_addSku}`) : ($.getdata('guaopencard3') ? $.getdata('guaopencard3') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard3 ? process.env.guaopencard3 : `${guaopencard}`) : ($.getdata('guaopencard3') ? $.getdata('guaopencard3') : `${guaopencard}`);
...
guaopencard_draw = $.isNode() ? (process.env.guadraw3 ? process.env.guadraw3 : guaopencard_draw) : ($.getdata('guadraw3') ? $.getdata('guadraw3') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard3]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
