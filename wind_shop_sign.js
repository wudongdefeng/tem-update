...
!(async () => {
  ...
  var idNum = $.Token.split("&")
  console.log(`共`+idNum.length+'个店铺，开始签到...')
...
function getvenderName(venderId) {
  return new Promise(resolve => {
    const options = {
      ...
      url: `https://api.m.jd.com/client.action?functionId=whx_getMShopOutlineInfo&body=%7B%22venderId%22:%22${venderId}%22,%22source%22:%22m-shop%22%7D&t=${Date.now()}&appid=shop_view`,
      headers: {
        "accept": "*/*",
        "accept-encoding": "gzip, deflate, br",
        "accept-language": "zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7",
    ...
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          //console.log('getvenderName',data)
          data = JSON.parse(data)
          $.shopName = data.data.shopInfo.shopName
          $.shopId = data.data.shopInfo.shopId
          if ($.index == 1) {
    ...
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          //}
          if ($.index == 1) {
              console.log(`开始时间：`+timestampToTime($.startTime))
              console.log(`结束时间：`+timestampToTime($.endTime))
          }
          giftConditions = data.data.continuePrizeRuleList
          $.dayNums = []
          for (let info of giftConditions) {
              $.dayNum = info.level
              if (info.prizeList[0].userPirzeStatus == '1' && info.prizeList[0].type == '4')
              {
                  console.log(`签到${$.dayNum}天获得${info.prizeList[0].discount}京豆`)
                  $.dayNums.push($.dayNum)
                  $.signFlag = true
              }
          }
          if (data.data.prizeRuleList[0] && typeof data.data.prizeRuleList[0].prizeList[0].type != "undefined"){
              if (data.data.prizeRuleList[0].prizeList[0].type == '4' && data.data.prizeRuleList[0].prizeList[0].userPirzeStatus == '1'){
                  console.log("每日签到获得" + data.data.prizeRuleList[0].prizeList[0].discount + "京豆")
                  $.signFlag = true
              }
          }
