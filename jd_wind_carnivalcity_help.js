...
!(async () => {
  ...
  $.updatePkActivityIdRes = await getAuthorShareCode('https://codeberg.org/wudongdefeng/updateTeam/raw/branch/main/shareCodesjd_cityShareCodes.json')
  if (!$.updatePkActivityIdRes) {
    $.http.get({url: 'https://codeberg.org/wudongdefeng/updateTeam/raw/branch/main/shareCodesjd_cityShareCodes.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.updatePkActivityIdRes = await getAuthorShareCode('https://codeberg.org/wudongdefeng/updateTeam/raw/branch/main/shareCodesjd_cityShareCodes.json')
  }
