...
!(async () => {
  ...
  $.updatePkActivityIdRes = await getAuthorShareCode('https://codeberg.org/wudongdefeng/updateTeam/raw/branch/main/shareCodes/jd_cityShareCodes.json')
  if (!$.updatePkActivityIdRes) {
    $.http.get({url: 'https://codeberg.org/wudongdefeng/updateTeam/raw/branch/main/shareCodes/jd_cityShareCodes.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.updatePkActivityIdRes = await getAuthorShareCode('https://codeberg.org/wudongdefeng/updateTeam/raw/branch/main/shareCodes/jd_cityShareCodes.json')
  }
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://api.jdsharecode.xyz/api/carnivalcity`, 'timeout': 20000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
