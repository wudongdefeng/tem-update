...
class Env {
  ...
  async send() {
    if (this.msg?.length > 0) {
      this.msg.push(
        "运行时长：" + ((this.timestamp() - this.start) / 1000).toFixed(2) + "s"
      );
      if (this.bot) {
  ...
  async wait(min, max) {
    if (max) {
      return new Promise((resolve) =>
        setTimeout(resolve, this.random(min, max))
      );
    } else {
  ...
  build(url) {
    ...
    if (stk) {
      ...
      if (tk && fp && em) {
        ...
        tk =
          "tk01wcdf61cb3a8nYUtHcmhSUFFCfddDPRvKvYaMjHkxo6Aj7dhzO+GXGFa9nPXfcgT+mULoF1b1YIS1ghvSlbwhE0Xc";
        fp = "9686767825751161";
      ...
      stk.split(",").map((item, index) => {
        st += `${item}:${this.getQueryString(url, item)}${
      ...
      ens = encodeURIComponent(
        [
          ...
          "".concat(tk),
          "".concat(
            CryptoJS.HmacSHA256(st, hash.toString()).toString(CryptoJS.enc.Hex)
          ),
        ].join(";")
      );
      if (url.match(/[?|&]h5st=(.*?)&/)) {
        url = url
          .replace(url.match(/[?|&]h5st=(.*?)&/)[1], "H5ST")
          .replace(/H5ST/, ens);
      }
      let matchArr = [
        /[?|&]_time=(\d+)/,
  ...
  formatDate(date, fmt) {
    ...
    return format(
      typeof date === "object"
        ? date
        : new Date(typeof date === "string" ? date * 1 : date),
      fmt || "yyyy-MM-dd"
    );
  }
