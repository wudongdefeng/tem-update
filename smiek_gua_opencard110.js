...
3.关注5京豆
4.加购5京豆
  (默认不加购 如需加购请设置环境变量[guaopencard0]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guadraw0]为"3"
填写要抽奖的次数 不足已自身次数为准
guadraw0="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 1.21~1.28 年货不打烊 好物带回家 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkkkbblnt20220121A&shareUuid=c2b6865e5761403682f474a176eba015)]
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard0 ? process.env.guaopencard0 : `${guaopencard_addSku}`) : ($.getdata('guaopencard0') ? $.getdata('guaopencard0') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard0 ? process.env.guaopencard0 : `${guaopencard}`) : ($.getdata('guaopencard0') ? $.getdata('guaopencard0') : `${guaopencard}`);
...
guaopencard_draw = $.isNode() ? (process.env.guadraw0 ? process.env.guadraw0 : guaopencard_draw) : ($.getdata('guadraw0') ? $.getdata('guadraw0') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard0]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "c2b6865e5761403682f474a176eba015"
  ...
  try {
    ...
    if(!$.addCart && !$.outFlag){
      if(guaopencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置环境变量[guaopencard0]为"true"');
      }
    }
    if(flag){
      await takePostRequest('activityContent');
    ...
    }else console.log('如需抽奖请设置环境变量[guadraw0]为"3" 3为次数');
