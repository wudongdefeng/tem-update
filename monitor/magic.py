...
# 0. 进入容器
# 1. pip3 install -U cacheout telethon pysocks
# 2. 复制magic.py,magic.json到/ql/data/config/目录 并配置
# 3. python3 /ql/data/config/magic.py 用手机号登录
# 4. 给bot发送在吗 有反应即可
# 5. pm2 start /ql/data/config/magic.py -x --interpreter python3
# 6. 挂起bot到后台 查看状态 pm2 l
# 7. 如果修改了magic.json,执行pm2 restart magic 即可重启
# pm2 start /jd/data/config/magic.py -x --interpreter python3
...
async def handler(event):
    ...
    logger.info(f"最原始数据 {origin}")
    text = re.search(f'((export\s)?\w*=(".*"|\'.*\'))',parse.unquote_plus(parse.unquote_plus(origin)))[1]
    groupname = "mybot"
    try:
    ...
    try:
        ...
        if tmp_text != text:
            await client.send_message(bot_id, f'转换后数据 {text}')
        activity_id, url = await get_activity_info(text)
        #logger.info(f"缓存数据 {text}")
...
async def converter_handler(text):
    ...
    await client.send_message(bot_id, f'转换前数据 {text}')
    try:
        tmp_text = text
        # 转换
        for c_key in monitor_converters:
            ...
            if result is None:
                ...
                continue
            url2id = re.search("url2id", c_key)
            if url2id is not None:
                logger.info(f"将url转换为id {text}")
                rule = monitor_converters.get(c_key)
                target = rule.get("env")
                activityId = await get_activityId(text)
