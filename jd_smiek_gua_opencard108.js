...
3.关注5京豆
4.加购5京豆
  (默认不加购 如需加购请设置环境变量[guaopencard8]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guadraw8]为"3"
填写要抽奖的次数 不足已自身次数为准
guadraw8="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 1.18~1.24 会员联合盛典 年货好物集结 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkklblsifkpeq0118&shareUuid=7735bb52cc604c84a6abc21237b653c9)]
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard8 ? process.env.guaopencard8 : `${guaopencard_addSku}`) : ($.getdata('guaopencard8') ? $.getdata('guaopencard8') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard8 ? process.env.guaopencard8 : `${guaopencard}`) : ($.getdata('guaopencard8') ? $.getdata('guaopencard8') : `${guaopencard}`);
...
guaopencard_draw = $.isNode() ? (process.env.guadraw8 ? process.env.guadraw8 : guaopencard_draw) : ($.getdata('guadraw8') ? $.getdata('guadraw8') : guaopencard_draw);
...
!(async () => {
  ...
  $.shareUuid = "7735bb52cc604c84a6abc21237b653c9"
  ...
  try {
    ...
    if(!$.addCart && !$.outFlag){
      if(guaopencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置环境变量[guaopencard8]为"true"');
      }
    }
    if(flag){
      await takePostRequest('activityContent');
    ...
    }else console.log('如需抽奖请设置环境变量[guadraw8]为"3" 3为次数');
