      ...
      try {
        if (err) {
          ...
          if (data) {			
            ...
            $.message += `\n当前总红包：${$.balance}(今日总过期${$.expiredBalance})\n京喜红包：${$.jxRed}(今日将过期${$.jxRedExpire.toFixed(2)})\n极速红包：${$.jsRed}(今日将过期${$.jsRedExpire.toFixed(2)})\n京东红包：${$.jdRed}(今日将过期${$.jdRedExpire.toFixed(2)})`;
          } else {
            console.log(`京东服务器返回空数据`)
          }
        }
