...
2.开6张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[guaopencard4]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guadraw4]为"3"
填写要抽奖的次数 不足已自身次数为准
guadraw4="3"
填非数字会全都抽奖
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard4 ? process.env.guaopencard4 : `${guaopencard_addSku}`) : ($.getdata('guaopencard4') ? $.getdata('guaopencard4') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard4 ? process.env.guaopencard4 : `${guaopencard}`) : ($.getdata('guaopencard4') ? $.getdata('guaopencard4') : `${guaopencard}`);
...
guaopencard_draw = $.isNode() ? (process.env.guadraw4 ? process.env.guadraw4 : guaopencard_draw) : ($.getdata('guadraw4') ? $.getdata('guadraw4') : guaopencard_draw);
...
async function run() {
  try {
    ...
    if(guaopencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[guaopencard4]为"true"');
    }
    await takePostRequest('activity_load');
    ...
    }else console.log('如需抽奖请设置环境变量[guadraw4]为"3" 3为次数');
