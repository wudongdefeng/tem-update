...
const inviteCodes = ['T019_qwtFEtHolbeIRv3lP8CjVWnYaS5kRrbA', 'T0225KkcR09Lo1TVIhullfVedwCjVWnYaS5kRrbA', 'T0205KkcJ2x4nB6lQ0aH76FwCjVWnYaS5kRrbA'];
!(async () => {
  await requireConfig();
  if (!cookiesArr[0]) {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://cdn.jsdelivr.net/gh/wudongdefeng/jd-temporary@main/wind_ddfactory.json`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
          console.log(`${$.name} API请求失败，请检查网路重试`)
