...
(默认不加购 如需加购请设置环境变量[guaopencard]为"true"
...
!(async () => {
  ...
  if(guaopencard+"" != "true"){
    console.log('如需开卡请设置环境变量[guaopencard]为"true"')
  }
  ...
  try {
    ...
    for(let i in config){
      ...
      if($.task.memberTask && $.task.memberTask.memberList){
        let msg = ''
        if(guaopencard+"" != "true"){
          msg = '如需开卡请设置环境变量[guaopencard]为"true"'
        }
      ...
      if($.task.dailyTask && $.task.dailyTask.taskList){
        for (let t in $.task.dailyTask.taskList) {
          ...
          if(guaopencard_addSku+"" != "true" && 2 == $.oneTask.groupType) console.log('如需加购请设置环境变量[guaopencard]为"true"\n');
