  ...
  await client.ListFunctions({}).then(
    async data => {
      let func = data.Functions.filter(
        vo => vo.FunctionName === process.env.TENCENT_FUNCTION_NAME1
      );
      ...
      if (func.length) {
        ...
        // 更新代码，删除后重建
        params = {
          FunctionName: process.env.TENCENT_FUNCTION_NAME1
        };
