...
直播红包雨
参考shylocks大佬的脚本改的
脚本更新地址：https://raw.githubusercontent.com/monk-coder/dust/dust/backup/red_rain.js
脚本测试兼容: Quantumult X
==========Quantumult X==========
[task_local]
#京东直播雨
...
function taskUrl(function_id, body = {}) {
    return {
        url: `https://api.m.jd.com/api?functionId=${function_id}&body=${escape(JSON.stringify(body))}&client=wh5&clientVersion=1.0.0&_=${new Date().getTime() + new Date().getTimezoneOffset() * 60 * 1000 + 8 * 60 * 60 * 1000}`,
        headers: {
            "Accept": "*/*",
...
function getRedRainBody() {
    return new Promise(resolve => {
        $.get({ url: `https://api.r2ray.com/jd.redRain/index?ts=${(new Date).getTime()}` }, (err, resp, data) => {
            try {
                if (err) {
                    $.log(`请求异常：${err}`)
                } else {
