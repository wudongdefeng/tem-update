...
!(async () => {
  ...
  await requireConfig()
  $.authorCode = await getAuthorShareCode('https://codeberg.org/wudongdefeng/updateTeam/raw/branch/main/shareCodes/jd_updateCash.json')
  if (!$.authorCode) {
    $.http.get({url: 'https://codeberg.org/wudongdefeng/updateTeam/raw/branch/main/shareCodes/jd_updateCash.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorCode = await getAuthorShareCode('https://codeberg.org/wudongdefeng/updateTeam/raw/branch/main/shareCodes/jd_updateCash.json') || []
  }
