...
All变量适用
————————————————
入口：[ 1.14~1.20 大牌会员日 (https://lzdz1-isv.isvjcloud.com/dingzhi/bigname/memberday/activity?activityId=dz2201100008884901&shareUuid=3b573097bb2140cd8842e486632d1f87)]
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard7 ? process.env.guaopencard7 : `${guaopencard_addSku}`) : ($.getdata('guaopencard7') ? $.getdata('guaopencard7') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard7 ? process.env.guaopencard7 : `${guaopencard}`) : ($.getdata('guaopencard7') ? $.getdata('guaopencard7') : `${guaopencard}`);
...
guaopencard_draw = $.isNode() ? (process.env.guadraw7 ? process.env.guadraw7 : guaopencard_draw) : ($.getdata('guadraw7') ? $.getdata('guadraw7') : guaopencard_draw);
...
!(async () => {
  ...
  $.shareUuid = "3b573097bb2140cd8842e486632d1f87"
...
async function run() {
  try {
    ...
    }else console.log('如需抽奖请设置环境变量[guadraw7]为"3" 3为次数');
