...
if (process.env.TTTQDTK) {
  if (process.env.TTTQDTK.includes('\n')) {
    ...
    token = [...process.env.TTTQDTK.split('&'),...token]
  }
}
if (!token.length) {
  console.log('无通天塔签到token,不执行.需自备token:环境变量TTTQDTK: tk1&tk2.')
  return
}
...
async function main() {
  for (var j = 0; j < token.length; j++) {
    ...
    if (token[j]=='') {continue}
    //$.token=token[j]
    let activeIdInfo = await activeId(token[j]);
    if (activeIdInfo) {
        try {
            e = JSON.parse(activeIdInfo.activityData.floorList[0].boardParams.interaction);
        } catch (e) {
        }
        //e = JSON.parse(activeIdInfo.activityData.floorList[0].boardParams.interaction);
        ...
        if (e) {
            let taskInfo = await task('doInteractiveAssignment', { "encryptProjectId": e.encryptProjectId, "encryptAssignmentId": e.encryptAssignmentId, "completionFlag": true, "itemId": "1", "sourceCode": "aceaceqingzhan" }, token[j]);
...
function task(functionId, body, token) {
    let opt = {
        ...
        headers: {
            ...
            "Accept": "*/*",
            "User-Agent": UA,
            "Referer": `https://prodev.m.jd.com/mall/active/${token}/index.html`,
            "Content-Length": "270",
...
function activeId(token) {
    return new Promise((resolve) => {
        const option = {
            url: `https://prodev.m.jd.com/mall/active/${token}/index.html`,
            headers: {
