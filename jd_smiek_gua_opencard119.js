...
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[guaopencard9]为"true"
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guadraw9]为"3"
填写要抽奖的次数 不足已自身次数为准
guadraw9="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 3.17~3.25 大牌联合 好物焕新季 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkk068d4d0ab8a6609723002f50&shareUuid=ef7d83654bd742719c595e05d698ddf8)]
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard9 ? process.env.guaopencard9 : `${guaopencard_addSku}`) : ($.getdata('guaopencard9') ? $.getdata('guaopencard9') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard9 ? process.env.guaopencard9 : `${guaopencard}`) : ($.getdata('guaopencard9') ? $.getdata('guaopencard9') : `${guaopencard}`);
...
guaopencard_draw = $.isNode() ? (process.env.guadraw9 ? process.env.guadraw9 : guaopencard_draw) : ($.getdata('guadraw9') ? $.getdata('guadraw9') : guaopencard_draw);
...
!(async () => {
  ...
  $.shareUuid = "ef7d83654bd742719c595e05d698ddf8"
  ...
  try {
    ...
    if(!$.addCart && !$.outFlag){
      if(guaopencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置环境变量[guaopencard9]为"true"');
      }
    }
    if(flag){
      await takePostRequest('activityContent');
    ...
    }else console.log('如需抽奖请设置环境变量[guadraw9]为"3" 3为次数');
