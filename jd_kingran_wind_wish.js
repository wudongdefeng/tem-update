...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      message = '';
      //await TotalBean();
      console.log(`\n111*开始【京东账号${$.index}】${$.nickName || $.UserName}1111*\n`);
      if (!$.isLogin) {
  ...
  let res = await getAuthorShareCode('https://codeberg.org/wudongdefeng/updateTeam/raw/branch/main/shareCodes/wish.json')
