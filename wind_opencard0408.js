...
async function run() {
  try {
    ...
    let config = [
	{configCode:'35e7d1e312954db98308c658b8f6def5',configName:'京东文具'},
	]
    for(let i in config){
      $.hotFlag = false
      let item = config[i]
...
async function getUA(){
  ...
  let arr = await Faker.getBody($.UA,'https://prodev.m.jd.com/mall/active/4TNYnd1UBnS5aZivzCsqyFfjdSN7/index.html')
  $.fp = arr.fp
  await getEid(arr)
}
