...
2.开4张卡 成功开1张 抽1次将
3.加购5京豆
  (默认不加购 如需加购请设置环境变量[guaopencard1]为"true"
4.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guadraw1]为"3"
5.关注10京豆
6.关注频道10京豆
填写要抽奖的次数 不足已自身次数为准
guadraw1="3"
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard1 ? process.env.guaopencard1 : `${guaopencard_addSku}`) : ($.getdata('guaopencard1') ? $.getdata('guaopencard1') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard1 ? process.env.guaopencard1 : `${guaopencard}`) : ($.getdata('guaopencard1') ? $.getdata('guaopencard1') : `${guaopencard}`);
...
guaopencard_draw = $.isNode() ? (process.env.guadraw1 ? process.env.guadraw1 : guaopencard_draw) : ($.getdata('guadraw1') ? $.getdata('guadraw1') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard1]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
  ...
  try {
    ...
    if(!$.followSku && !$.outFlag){
      if(guaopencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置环境变量[guaopencard1]为"true"');
      }
    }
    if(flag){
      await takePostRequest('activityContent');
    ...
    }else console.log('如需抽奖请设置环境变量[guadraw1]为"3" 3为次数');
